...
public class SessionService {
  ...
  public Session create(UUID userId) {
    ...
    return new Session(token, Instant.now().plus(ttl));
  }

  public Optional<UUID> resolve(String token) {
    try {
      return Optional.ofNullable(redis.opsForValue().get(PREFIX + UUID.fromString(token)))
          .map(UUID::fromString);
  ...
  public void delete(String token) {
    try {
      redis.delete(PREFIX + UUID.fromString(token));
