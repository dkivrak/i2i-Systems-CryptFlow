...
public class TradeService {
  ...
  @Transactional public TradeResult execute(UUID userId,AssetSymbol symbol,TradeSide side,BigDecimal rawQuantity){
    if(rawQuantity==null||rawQuantity.signum()<=0||Math.max(0,rawQuantity.stripTrailingZeros().scale())>MAX_DECIMAL_PLACES)throw invalidAmount();
    ...
    if(side==TradeSide.BUY){
      if(wallet.getUsdBalance().compareTo(total)<0)throw new ApiException(HttpStatus.UNPROCESSABLE_ENTITY,"INSUFFICIENT_FUNDS","Insufficient USD balance for this transaction.");
      wallet.setUsdBalance(wallet.getUsdBalance().subtract(total));
      asset.setQuantity(asset.getQuantity().add(quantity));
    }else{
      if(asset.getQuantity().compareTo(quantity)<0)throw new ApiException(HttpStatus.UNPROCESSABLE_ENTITY,"INSUFFICIENT_ASSET_BALANCE","Insufficient asset balance for sale.");
  ...
  private ApiException invalidAmount(){return new ApiException(HttpStatus.BAD_REQUEST,"INVALID_AMOUNT","Quantity must be positive and have at most "+MAX_DECIMAL_PLACES+" decimal places.");}
