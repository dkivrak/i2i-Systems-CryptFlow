...
public class MarketPriceService {
  private static final String KEY="market:prices"; private static final String UPDATED="updatedAt";
  private final StringRedisTemplate redis;
  public MarketPriceService(StringRedisTemplate redis){this.redis=redis;}
  public MarketPrices getCurrent(){
    var entries=redis.opsForHash().entries(KEY);var prices=new LinkedHashMap<String,BigDecimal>();
    ...
    var updated=entries.get(UPDATED);if(updated==null)throw unavailable();
    return new MarketPrices(prices,Instant.parse(updated.toString()));
  }
  public BigDecimal price(AssetSymbol symbol){return getCurrent().prices().get(symbol.name());}
