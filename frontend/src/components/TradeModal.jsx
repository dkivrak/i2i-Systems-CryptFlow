...
export default function TradeModal({ symbol, lockedPrice, usdBalance, assetQuantity, onClose, onComplete }) {
  ...
  async function submit(event) {
    ...
    if (side === 'BUY' && estimatedTotal > usdBalance) {
      setError(`Insufficient funds. Available: ${money(usdBalance)}`)
      return
    }

    if (side === 'SELL' && numericQuantity > assetQuantity) {
      setError(`Insufficient asset balance. Available: ${assetQuantity} ${symbol}`)
      return
    }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <div role="dialog" aria-modal="true" aria-label={`${symbol} Transaction`} className="card w-full max-w-md rounded-3xl p-7 animate-in">
      <div className="flex items-start justify-between">
        <div><p className="label">NEW ORDER</p><h2 className="mt-1 text-3xl font-black">{symbol} Transaction</h2></div>
      ...
      <div className="mt-5 flex items-center justify-between rounded-xl bg-[#081522] px-5 py-3">
        <span className="text-xs text-slate-500 uppercase tracking-wider">Locked price</span>
      ...
      {!canBuy && !canSell && <p className="mt-4 rounded-xl bg-amber-500/10 p-3 text-sm text-amber-300">You must have USD balance or crypto assets to perform a transaction.</p>}

      <form onSubmit={submit} className="mt-6">
        <label htmlFor="trade-quantity" className="text-sm text-slate-300">Coin quantity</label>
        ...
        <div className="mt-3 text-xs text-slate-500">
          {side === 'BUY'
            ? <span>Available: <span className="text-slate-300">{money(usdBalance)}</span></span>
            : <span>Owned: <span className="text-slate-300">{assetQuantity} {symbol}</span></span>}
        </div>

        {error && <p role="alert" className="mt-4 rounded-xl bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}
        <button disabled={busy || (!canBuy && side === 'BUY') || (!canSell && side === 'SELL')} className="btn btn-primary mt-6 w-full">{busy ? 'Processing order...' : 'Execute Order'}</button>
      </form>
    </div>
  </div>
}
